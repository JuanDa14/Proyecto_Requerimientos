...
class ImpuestoController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $fecha = Carbon::parse($request->txtfecha);
        $existe = DB::table('impuestos')->whereYear('fecha', $fecha->year)->whereMonth('fecha', $fecha->month)->get();
        if (count($existe) > 0) {
            return back()->with('error', 'Ya se ha registrado un impuesto en el periodo: '.$fecha->day. '-' . $fecha->month . '-' . $fecha->year);
        } else {
            ...
            $impuesto->save();
            return redirect()->route('impuestos.index')->with('realizada', 'OK');
        }
    }
    ...
    public function imprimirAnual(Request $request)
    {
        $año = $request->año;
        $datos = DB::table('impuestos')->whereYear('fecha','=',$año)->get(); 
        $total = DB::table('impuestos')->whereYear('fecha','=',$año)->sum('monto'); 
        $pdf = PDF::loadView('impuestos.imprimir.reporte', compact('datos','total','año'));
        return $pdf->download('reporte_Impuesto_Anual_' . $año . '.pdf');
    }    
}
